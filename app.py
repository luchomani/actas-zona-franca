...
"Actas de Inventario e Inconsistencias", extraer 16 campos estandarizados
...
def extraer_campos_acta(texto: str, nombre_archivo: str) -> dict:
    ...
    un diccionario con los 16 campos estandarizados + metadatos de QA.
    """
    faltantes = []
    ...
    # 2 y 4. Documento de transporte y N°FMM (fila de la tabla de mercancía)
    doc_fmm = re.search(
        r"([A-Z]{2,10}\d{5,10})\s+(\d{6,10})\s+\d+\s+BULTOS", texto, re.IGNORECASE
    )
    if doc_fmm:
        documento_transporte = doc_fmm.group(1).strip()
        n_fmm = doc_fmm.group(2).strip()
    else:
        documento_transporte, n_fmm = "", ""
        faltantes += ["Documento de transporte", "N°FMM"]

    # 3. Tránsito / DTA (se toma el primer número largo tras "Número")
    transito = campo(
        "Transito",
        r"[Nn][uú]mero\s+(\d{10,20})\s+(?:se procede|de la aduana)",
    )

    # 5. N°FECHA -> fecha de generación del acta (tomada de Observaciones)
    n_fecha = campo(
        "N°FECHA",
        r"FECHA\s*\n?\s*GENERACI[ÓO]N\s+DEL\s+ACTA:?\s*(\d{2}/\d{2}/\d{4})",
    )
    if not n_fecha:
        # Alternativa: fecha impresa en el pie de página del PDF
        n_fecha = campo(
            "N°FECHA",
            r"(\d{1,2}/\d{1,2}/\d{2,4})\s+\d{1,2}:\d{2}\s*[AP]M",
        )

    # 6. Ingreso último vehículo (fecha de ingreso en la fila de desprecintaje)
    ingreso_vehiculo = campo(
        "INGRESO ÚLTIMO VEHÍCULO",
        r"\d{2}/\d{2}/\d{4}\s+[A-Z]{2,4}\d{5,8}\s+[A-Z0-9]{4,8}\s+(\d{2}/\d{2}/\d{4})",
    )

    # 7. Fecha de autorización de tránsito (formato YYYY/MM/DD tal cual el PDF)
    fecha_autorizacion = campo(
        "Fecha de autorización Tránsito",
        r"autorizaci[óo]n de la operaci[óo]n\s*\(YYYY/MM/DD\)\s*(\d{4}/\d{2}/\d{2})",
    )

    # 8. Fecha máxima de finalización del régimen
    fecha_max_finalizacion = campo(
        "Fecha Maxima Finalización",
        r"l[ií]mite para finalizar el r[ée]gimen\s*\(YYYY/MM/DD\)\s*(\d{4}/\d{2}/\d{2})",
    )

    # 9. Número de Acta (PICIZ)
    acta_piciz = campo(
        "Acta de Inventario e Inconsistencias PICIZ",
        r"Acta N\.?\s*(\d+)",
    )

    # 10. Fecha del acta de inventario (misma fecha de generación)
    fecha_acta = n_fecha if n_fecha else campo(
        "Fecha acta de inventario e inconsistencias",
        r"generad[ao]?\s*:?\s*(\d{2}/\d{2}/\d{4})",
    )

    # 11. No. Planilla de recepción -> no existe en este formato de acta
    no_planilla = "N/A"

    # 12. Peso báscula ZFC (peso total de la sección TOTALES)
    peso_bascula = campo(
        "Peso Báscula ZFC",
        r"TOTALES:?\s*[\d.,]+\s+([\d.,]+)",
    )
    ...
    # 13 y 14. Campos de diligenciamiento manual -> siempre vacíos
    bitacora = ""
    rad_solicitud = ""

    # 15. Observaciones / Inconsistencias
    ...
    # 16. SINO -> depende de si la sección "Inconsistencias" reporta N/A
    inconsistencias_valor = _buscar(r"Inconsistencias:\s*([^\n]+)", texto)
    if inconsistencias_valor is None:
        # Formato alterno: bloque con Bultos/Estado/Descripción/Términos = N/A
        bloque = _buscar(
            r"Bultos\s*(.+?)Otra\s*[:\-]?\s*(SI|NO|N/A)",
            texto,
            grupo=0,
        ) or ""
        sino = "NO" if "N/A" in bloque.upper() or bloque == "" else "SI"
    else:
        sino = "NO" if inconsistencias_valor.strip().upper() == "N/A" else "SI"
    ...
    return {
        ...
        "N°FMM": n_fmm,
        "N°FECHA": n_fecha,
        "INGRESO ÚLTIMO VEHÍCULO": ingreso_vehiculo,
        ...
        "No. Planilla de Recepción": no_planilla,
        "Peso Báscula ZFC": peso_bascula,
        "BITACORA N°": bitacora,
        "RAD SOLICITUD BITACORA": rad_solicitud,
...
st.caption("Zona Franca · Extracción automática de 16 campos estandarizados desde PDF")
...
if not df.empty:
    ...
    with col_b:
        solo_inconsistencias = st.checkbox("Solo con inconsistencias (SI)", value=False)
    ...
    if solo_inconsistencias:
        df_vista = df_vista[df_vista["SINO"] == "SI"]
